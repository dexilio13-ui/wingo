...
REM   3/5  pokrece GitHub Action workflow i ceka njegov kraj (--wait)
REM   4/5  watch prozor - novo kolo se automatski dodaje (~4 min)
REM   5/5  server + automatsko otvaranje browsera sa menijem
REM
REM  JSON cuva zadnjih 60 kola. Prvo pokretanje ucita ~10,
...
REM  --wait: ceka kraj workflow-a i ispisuje status (do ~5 min)
call npm run trigger -- --wait
if errorlevel 1 (
    echo        Trigger nije prosao - Actions i dalje rade sami po cron-u na 5 min.
) else (
    echo        Workflow zavrsen - feed je osvezen.
)
...
echo.
echo  ============================================================
echo   Sve radi! Prikaz zadnjih 60 kola:  http://localhost:8080
